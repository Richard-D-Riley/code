...
# Fully simulation based - logistic regression
# ------------------------------------------------------------------------------
...
for (i in 1:sim) {
  ...
  fit <- try(glm(outcome ~ ageyrssd + ga_diag1sd + hist2sd + hist3sd + pcr1sd +
                   serurea11sd + pcsd + sbpsd + trt_ahsd + trt_mgso4sd,
                 data = dev_sample, family = binomial), silent = TRUE)
             
  
  # Heuristic shrinkage
  chi2 <- fit$null.deviance - fit$deviance
  df_m <- length(coef(fit)) - 1
  performance[i, 6] <- (chi2 - df_m) / chi2
  rm(chi2, df_m)
  
  
  # Predicted probabilities in dev data
  dev_sample$p_dev <- predict(fit, type = "response")
  ...
  RR_eval$lin_pred <- predict(fit, newdata = RR_eval, type = "link")
  RR_eval$prob <- predict(fit, newdata = RR_eval, type = "response")
