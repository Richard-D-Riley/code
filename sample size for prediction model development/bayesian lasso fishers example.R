...
# Bayesian approximation via Fisher’s information - Lasso logistic regression
# ------------------------------------------------------------------------------
...
# Bayesian model with lasso prior in Stan
...
model {
  ...
  for (j in 2:P)
    beta[j] ~ double_exponential(0, lambda);
...
linpred_dev <- X_dev %*% t(betas)
pred_dev <- plogis(linpred_dev)
