...
* fit lasso model with the candidate predictors using the sample 
* use 10-fold CV used by default to minimise the loss function
* use capture so that it will go to the next loop if there is an error
 capture elasticnet logit outcome ageyrssd ga_diag1sd hist2sd hist3sd pcr1sd serurea11sd pcsd sbpsd trt_ahsd trt_mgso4sd, alphas(1)
...
capture matrix b[1,1] = B["y1","ageyrssd"]
...
* only want the first row of results for the chosen 0.5 threshold
use test, replace 
drop if _n > 1  
...
* fit LP for developed model 
gen lin_pred = b[1,11] + b[1,1]*ageyrs + b[1,2]*ga_diag1sd + b[1,3]*hist2sd + b[1,4]*hist3sd + b[1,5]*pcr1sd + b[1,6]*serurea11sd + b[1,7]*pcsd + b[1,8]*sbpsd + b[1,9]*trt_ahsd + b[1,10]*trt_mgso4sd
