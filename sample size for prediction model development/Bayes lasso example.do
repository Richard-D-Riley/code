*** CODE FOR THE PRE-ECLAMPSIA EXAMPLE IN SECTION 3 (Results) OF RILEY ET AL. 
...
* sample N observations, the sample size - in paper we consider various choices
* local N = 75
* local N = 335
local N = 456
* local N = 1000
...
* fit Bayes lasso model to the candidate priors using the sample
...
prior({outcome:_cons} , normal(0, 1e6))  ///
 prior( {outcome: ageyrssd ga_diag1sd hist2sd hist3sd pcr1sd serurea11sd pcsd sbpsd trt_ahsd trt_mgso4sd} , laplace(0, (sqrt(1/{lam2})))) ///
 prior({lam2=1},  gamma(1, 1/1.78)) ///
 dots saving(simdata, replace) thinning(10) hpd ///
 block({outcome: ageyrssd ga_diag1sd hist2sd hist3sd pcr1sd serurea11sd pcsd sbpsd trt_ahsd trt_mgso4sd}) block({lam2}) ///
adaptation(every(50))  mcmcsize(1000) burnin(5000)
...
* only want the first row of results for the threshold 0.5 of interest
use test, replace 
drop if _n > 1  
