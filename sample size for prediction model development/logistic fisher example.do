...
*** this code is for implementing the Fisher's decompoisition approximation for logistic regression
...
* now we can randomly draw directly from this mvnormal distribution to create our simulated logsitic regression models
* that is, we can draw parameter estimates conditional on the user's chosen sample size avoce 
* this avoids the fully simulation based approach

	frame create parameters
frame change parameters

* generate parameter estimates (provides an equation in a row of data) from mvnormal with specific mean and var-cov matrix
drawnorm eq1-eq11, means(estimates) cov(varcov_new) n(1000) seed(66) clear

* create predictions from these parameter estimates in the evaluation data
mkmat eq1 eq2 eq3 eq4 eq5 eq6 eq7 eq8 eq9 eq10 eq11, matrix(parameters)
...
mata: predict = invlogit(D*parametersmata)

forvalues j = 1/10 {
	
mata: st_view(l`j'=., ., st_addvar("double", "p_bs`j'"))
mata: l`j'[., 1] = predict[.,`j''] 
...
frame put p p_bs* outcome, into(predictions)
frame put p lp* outcome, into(lin_predictors)


* create matrix to store performance measures
matrix performance = J(10,5,.)
...
gen model=(p_bs`i'>z)*(outcome-(1-outcome)*z/(1-z)) 
qui sum model
...
* Net benefit
gen all = outcome-(1-outcome)*z/(1-z)
sum all
local ENBall = r(mean) 

*NB of using the correct risks
gen correct = (p>z)*(outcome-(1-outcome)*z/(1-z))
sum correct
local ENBmax = r(mean)

* C stat max
 pmcstat p outcome
local cstat_max = r(cstat)
...
* true model c-stat is about 0.755 in eval data, so work out degradation
gen cstat_degrad = cstat - `cstat_max'
...
* degradation of developed model (true model net benefit is about 0.40552 in eval data )
 gen NBmodel_degrad = NBmodel - `ENBmax'
nois  summ NBmodel_degrad
nois  centile NBmodel_degrad, centile(2.5 50 97.5)
 
 gen NBmodel_degrad_percent = (100*(NBmodel/`ENBmax'))
