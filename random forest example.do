...
/* NB based on a threshold of 0.7

* net benefit of the developed model
 sum NBmodel
 centile NBmodel, centile(2.5 50 97.5)
 
* degradation of developed model (true model net benefit is 0.241 )
gen NBmodel_degrad = NBmodel - 0.241
 summ NBmodel_degrad
 centile NBmodel_degrad, centile(2.5 50 97.5)
 
 gen NBmodel_degrad_percent = (100*(NBmodel/0.241))
summ NBmodel_degrad_percent
centile NBmodel_degrad_percent, centile(2.5 50 97.5)
...
gen NB_winner = NBmodel
replace NB_winner = 0 if strategy == 0
replace NB_winner = -0.062 if strategy == 1

summ NB_winner
centile NB_winner, centile(2.5 50 97.5)

gen NB_winner_degrad = NB_winner - 0.241 
summ NB_winner_degrad
centile NB_winner_degrad, centile(2.5 50 97.5)

gen NB_winner_degrad_percent =  (100*(NB_winner/0.241))
summ NB_winner_degrad_percent
centile NB_winner_degrad_percent, centile(2.5 50 97.5)
...
**** ALTERNATIVE VERSION THAT AVOIDSTRICK TO GET THE PREDICTIONS IN THE EVAL DATA

***** When moving from the development data to the evaluation data, the rforest predict did not seem to be working properlyin the above code as it 
***** seemed to put the prob event in the prob of noevent column and the prob of event column was set a .
***** To resolve that I simply copied the P(no event) to the P(event) col to make it work. 
***** For peace of mind, I then created this altnerative simulation code below, which uses a single datatse after combining the dev and eval datasets
***** - the results are basically identical to the above, and so gives me reassurance that the 'trick' i used above works
***** the below is also slower
...
* create frame for development data
capture frame create dev
* frame dev: use "/Users/rileyrd/Library/CloudStorage/OneDrive-UniversityofBirmingham/Work/Grants awarded/Richard Riley - SS-PREDICT/SS based on stability/Bayes/PREP example/RR_dev.dta", clear


use "/Users/rileyrd/Library/CloudStorage/OneDrive-UniversityofBirmingham/Work/Grants awarded/Richard Riley - SS-PREDICT/SS based on stability/Bayes/PREP example/RR_dev.dta", clear

rename true_p_new p

gen dev_yes = 1
gen eval_yes = 0
* no need to keep huge amounts of the 800k data to sample from as it slows things down
drop if _n>10000

save dev_temp, replace

use "/Users/rileyrd/Library/CloudStorage/OneDrive-UniversityofBirmingham/Work/Grants awarded/Richard Riley - SS-PREDICT/SS based on stability/Bayes/PREP example/RR_eval.dta", clear
drop if _n>100000
gen dev_yes = 0
gen eval_yes = 1
rename true_p_new p

save eval_temp, replace

append using dev_temp, 

save overall , replace


capture frame create overall
frame overall: use overall.dta 


frame change overall

* rename true_p_new p

* no need to keep the 800k records when the sample size is only going to look at values up to about 1000

* drop if _n > 12000

*create the test set
* gen eval_yes = 0
* replace eval_yes = 1 if _n <= 10000



/* create frame for evaluation data
capture frame create eval
frame eval: use "/Users/rileyrd/Library/CloudStorage/OneDrive-UniversityofBirmingham/Work/Grants awarded/Richard Riley - SS-PREDICT/SS based on stability/Bayes/PREP example/RR_eval.dta", clear
frame change eval
set seed 66
*sample 100000, count
drop if _n>100000
...
* sort to ensure the development data is first and sorted by random number (lowest values first)
sort eval_yes random
gen dev_yes_temp = 0
replace dev_yes_temp = 1 if _n <= `N'
...
* iterations(100) = 100 trees;  type(class) indicates it is a classification problem
* not a continuous outcome (which uses type(reg))

 capture rforest outcome ageyrssd ga_diag1sd hist2sd hist3sd pcr1sd serurea11sd pcsd sbpsd trt_ahsd trt_mgso4sd if dev_yes_temp == 1, type(class) iterations(100) depth(3)
 ...
 * if there is no error then work out the results
 ...
 gen p_eval = p_dev if eval_yes == 1
 
 replace p_dev = . if eval_yes == 1
 
 replace p_dev = . if dev_yes_temp != 1
...
* the following works out the net benefit of the three strategies at a relevant threshold in the development data
* dca outcome p_dev, nograph  xstart(0.7) xstop(0.8) xby(0.1) saving(test, replace)
dca outcome p_dev if dev_yes_temp ==1, nograph  xstart(0.5) xstop(0.6) xby(0.1) saving(test, replace)
* only want the first row of results 
...
capture drop abs_p lin_pred p_eval pr_noevent model  dev_yes_temp random p_dev 

}


*** calculate uncertainty in model predictions (95% intervals of predictions)
frame copy overall predictions
frame change predictions
...
*save "C:\Users\whittlrl\OneDrive - University of Birmingham\Project work\SS based on posterior dist and model degradation (Richard)\Sim example\Results\logistic_predictions_`N'", replace
...
*save "C:\Users\whittlrl\OneDrive - University of Birmingham\Project work\SS based on posterior dist and model degradation (Richard)\Sim example\Results\logistic_performance_`N'", replace
